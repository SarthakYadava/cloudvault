...
- Organize files into virtual folders, apply up to 8 searchable tags, and rename
  documents without copying or moving their private S3 objects.
...
- Assign document requests with instructions, assignees, due dates, and
  categories plus `PENDING`, `SUBMITTED`, or `APPROVED` status tracking.
- Show pending, submitted, approved, and overdue request counts per workspace.
...
| `GET` | `/api/files?page=0&size=20&query=report&folder=Tax&tag=2026&sort=name&direction=asc` | Search, filter, and sort file metadata |
| `PATCH` | `/api/files/{id}/metadata` | Rename a file and update its folder and tags |
| `GET` | `/api/files/folders` | List the current user's virtual folders |
...
- Renames and organizes documents with folders and tags while keeping S3 object
  keys stable.
- Filters the vault by filename, tag, folder, date, name, and size.
...
- File renames and folder moves update metadata only; private S3 object keys
  remain immutable.
